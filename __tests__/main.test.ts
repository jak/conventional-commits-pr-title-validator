...
describe('run action', () => {
  ...
  test('should fail when event is not pull_request', async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith("Invalid event: push. This action can only be used on 'pull_request'");
  });
  ...
  test('should process a valid pull request title', async () => {
    ...
    expect(core.setOutput).toHaveBeenCalledWith('title', 'feat: add new feature');
  ...
  test('should process a valid pull request title with a scope and breaking change', async () => {
    ...
    expect(core.setOutput).toHaveBeenCalledWith('valid', 'true');
    expect(core.setOutput).toHaveBeenCalledWith('title', 'fix(main)!: undo the confrigulators');
    expect(core.setOutput).toHaveBeenCalledWith('type', 'fix');
    expect(core.setOutput).toHaveBeenCalledWith('scope', 'main');
    expect(core.setOutput).toHaveBeenCalledWith('subject', 'undo the confrigulators');
  ...
  test('should handle invalid pull request title and fail when fail_when_invalid is true', async () => {
    ...
    expect(core.error).toHaveBeenCalledWith(expect.stringContaining('Unable to parse title'));
    expect(core.setFailed).toHaveBeenCalledWith('Pull Request title is not valid');
  });
